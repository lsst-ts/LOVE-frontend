...
- Components add subscriptions dispatching `addGroup` action (status PENDING)
- If connection is opened, then all PENDING subscriptions are requested automatically by calling `_requestSubscriptions` (status REQUESTING)
- When confirmation is received, then they change to status SUBSCRIBED
- When unsubscribing, by calling `requestGroupRemoval`, then they change to UNSUBSCRIBING
- When confirmation of unsubscription arrive, then the subscription is removed from the subscription list. If `addGroup` is called for the unsubscribed subscription, before the unbuscription confirmation arrives, then the subscription is not removed from the subscription list and follows the normal subscription steps.
- When connection is closed `resetSubscriptions` is called, then all subscriptions change to PENDING. This way, when we open the connection again, we will be able to re-subscribe by calling `_requestSubscriptions`
