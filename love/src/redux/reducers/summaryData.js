...
export default function (state = initialState, action) {
  switch (action.type) {
    case UPDATE_LOG_MESSAGE_DATA: {
      ...
      const nonDuplicates = action?.messages?.filter((msg) => {
        return cscData?.messages?.findIndex((log) => {
    ...
    case UPDATE_ERROR_CODE_DATA: {
      ...
      const nonDuplicates = action?.errorCodeData?.filter((msg) => {
        return cscData?.errorCodeData?.findIndex((log) => {
          return log?.private_sndStamp?.value === msg?.private_sndStamp?.value;
        }) === -1;
      }) ?? [];
