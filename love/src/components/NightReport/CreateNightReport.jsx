...
function WeatherField({ report, weather, setWeather }) {
  ...
  const queryNarrativelogs = (date) => {
    setLoading(true);
    const fromDateTime = date.utc().set({
      hour: 12,
      minute: 0,
      second: 0,
      millisecond: 0,
...
function ObservatoryForm({ report, setReport }) {
  ...
  useEffect(() => {
    ...
    ManagerInterface.getCurrentNightReport('AuxTel').then((reports) => {
      if (reports.length > 0) {
        ...
        setWeather(report.weather_conditions);
        setAuxtelStatus(report.auxtel_status);
        setSimonyiStatus(report.simonyi_status);
  ...
  const handleAuxtelStatusChange = (newAuxtelStatus) => {
    setAuxtelStatus(newAuxtelStatus);
    setChangesNotSaved(true);
  };

  const handleSimonyiStatusChange = (newSimonyiStatus) => {
    setSimonyiStatus(newSimonyiStatus);
    setChangesNotSaved(true);
  };
  ...
  useEffect(() => {
    if (report?.date_sent) {
      ...
      ManagerInterface.getEFDMostRecentTimeseries(cscsPayload, 1, timeCutdate, efdInstance).then((efdResponse) => {
        if (efdResponse) {
          console.log('EFD response:', efdResponse);
...
function JiraOBSTicketsTable({ report }) {
  ...
  const queryTickets = (date) => {
    setLoading(true);
    const currentObsDay = getObsDayFromDate(date.subtract(1, 'day'));
...
function ObservatoryData({ report, observatoryState, cscStates }) {
  return (
    <div className={styles.observatoryDataContainer}>
      <ConfluenceURLField />
      <TelescopesStates report={{ ...report, date_sent: '2025-06-04T00:00:00' }} observatoryState={observatoryState} />
