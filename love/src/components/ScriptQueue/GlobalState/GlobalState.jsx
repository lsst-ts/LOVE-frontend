  ...
  const MenuOption = ({ label, status }) => {
    ...
    const onToggleState = () => {
      ...
      if (status === OBSERVATORY_STATES.OPERATIONAL) {
        // Operational state is mutually exclusive with the other states: FAULT & WEATHER & DOWNTIME.
        updatedState =
        ...
        // FAULT and WEATHER states are mutually exclusive with OPERATIONAL state.
...
const GlobalState = ({
  ...
  const updateObservatoryState = useCallback(
    (state, note) => {
  ...
  const getContextMenuOptions = (element) => {
    ...
    if (element?.classList.contains('observatoryState')) {
      // Custom context menu for observatory states is rendered as children of the <ContextMenu/> component,
      // so we return an empty array here to avoid showing the default context menu options.
      return [];
    }
    return null;
  };
